...
本仓库已新增桌面端集群改造文档：[docs/cloud-cluster/README.md](docs/cloud-cluster/README.md)。

该规划把任务拆成多个 batch，由桌面端统一调度本地或远端 worker；worker 之间互不通信，客户端通过轮询获取 batch 状态、日志和结果，并把结果合并回同一个任务视图。远端 Docker worker 服务的独立文档项目位于 `domain-scanner-cloudserver`，两边可以同步或独立实现。
